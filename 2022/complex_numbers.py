...
class ArgandPlane(Scene):
    def construct(self):
        ...
        d1 = Dot(plane.n2p(0), radius = 0.08, color = YELLOW)
        p1 = Arrow(start = plane.n2p(- 0.25 - 0.05j), end = plane.n2p(4.30 + 1.05j), color = YELLOW, stroke_width = 3)
        lblp1 = MathTex("4+i").scale(0.5).next_to(p1, UR, 0.1)

        p2 = Arrow(start = plane.n2p(0.25 - 0.1j), end = plane.n2p(-4.30 + 2.15j), color = YELLOW, stroke_width = 3)
