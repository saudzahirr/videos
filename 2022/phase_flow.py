...
class PhaseFlowScene(Scene):
    ...
    def phase_flow(self, function, run_time, virtual_time = 2, rate_func = linear):
        self.play(
            PhaseFlow(
...
class Test(PhaseFlowScene):
    def construct(self):
        ...
        self.get_plane(show_axis = True, add_cordinate_labels = True)
        self.get_dots(color = YELLOW, show_creation = False)
